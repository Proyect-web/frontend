...
// 1. QUITAMOS 'active: true' DEL ARRAY. Lo calcularemos dinámicamente.
// Asegúrate de que estos IDs existan en tus secciones (ej: <section id="tienda">)
const menuItems = [
  { icon: Home, label: "Inicio", href: "/#inicio", id: "inicio" }, // Cambié href a #inicio para consistencia
  { icon: Ticket, label: "Caracteristicas", href: "/#caracteristicas", id: "caracteristicas" },
  { icon: HandCoins, label: "Ofertas", href: "/#ofertas", id: "ofertas" },
  { icon: ShoppingBag, label: "Tienda", href: "/#tienda", id: "tienda" },
  { icon: Gift, label: "Especificaciones", href: "/#especificaciones", id: "especificaciones" },
  { icon: TabletSmartphone, label: "App Movil", href: "/#app", id: "app" },
];
...
export default function Sidebar({ logoUrl, siteTitle }: SidebarProps) {
  ...
  // 3. LÓGICA DE SCROLL SPY (OBSERVADOR DE INTERSECCIÓN)
  useEffect(() => {
    // Opciones: 'rootMargin' define una línea imaginaria en el centro (-50%)
    // para decidir cuándo una sección se considera "activa".
    ...
    // Observamos todas las secciones definidas en el menú
  ...
  return (
    <>
        ...
        <Link href="/" className="flex items-center gap-2">
          {logoUrl ? (
            ...
            <div className="relative w-9 h-9 sm:w-10 sm:h-10 rounded-full bg-black/50 border border-white/10 flex items-center justify-center">
              <span className="text-base sm:text-lg font-black text-white">{displayInitials}</span>
            </div>
          )}
          <span className="text-lg sm:text-xl font-black text-white uppercase">{displayTitle}</span>
        </Link>
            ...
            >
              ...
              <div className="flex flex-col gap-3 p-4 sm:p-6">
                {menuItems.map((item, index) => {
                  // Verificación de activo para móvil
                  ...
                  return (
                    <Link 
                      ...
                      href={item.href} 
                      onClick={() => setIsMobileMenuOpen(false)} 
                      className={`flex items-center gap-4 p-3.5 sm:p-4 rounded-xl font-bold transition-all 
                        ${isActive 
                ...
                {extraItems.map((item, index) => (
                 ...
                 <Link href="/login" onClick={() => setIsMobileMenuOpen(false)} className="flex items-center justify-center gap-2 mt-4 p-3.5 sm:p-4 bg-[#0097B2] text-black font-bold rounded-xl hover:bg-[#0097B2]/90 transition-colors">
      ...
      <div className="hidden md:block">
        {/* TOP BAR Desktop */}
        <motion.div initial={{ y: -50, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ duration: 0.5 }} className="fixed top-4 mt-4 md:top-6 right-4 md:right-12 z-50 flex items-center gap-3 md:gap-4">
        ...
        <div className="fixed top-8 md:top-12 left-3 md:left-4 lg:left-6 z-50 flex flex-col gap-4 md:gap-5 lg:gap-6 items-start">
            ...
            <Link href="/" className="flex items-center gap-2 md:gap-3 lg:gap-4 pl-1 md:pl-2 mb-2 group">
              {logoUrl ? (
                <div className="relative w-4 h-4 md:w-8 md:h-8 lg:w-12 lg:h-12 rounded-full bg-black/50 border border-white/10 flex items-center justify-center overflow-hidden shadow-lg group-hover:scale-105 transition-transform">
                  <Image src={logoUrl} alt="Logo" width={24} height={24} className="object-contain md:w-[30px] md:h-[30px] lg:w-[36px] lg:h-[36px]" />
                </div>
              ) : (
                <div className="relative w-12 h-12 md:w-14 md:h-14 lg:w-16 lg:h-16 rounded-full bg-black/50 border border-white/10 flex items-center justify-center shadow-lg group-hover:scale-105 transition-transform">
                  <span className="font-bold text-white text-base md:text-lg lg:text-xl">{displayInitials}</span>
                </div>
              )}
              ...
              <AnimatePresence>
                {!hideLogoText && (
                  <motion.span
                    ...
                    className="text-lg md:text-xl lg:text-2xl font-black text-white tracking-tight drop-shadow-md uppercase"
                  >
                    {displayTitle}
                  </motion.span>
                )}
          ...
          <motion.nav
            ...
            initial={{ width: "100px" }} 
            animate={{ width: isHovered ? "220px" : "75px" }} 
          ...
          >
             ...
             {/* LISTA PRINCIPAL DINÁMICA */}
             <div className="flex flex-col gap-1.5 md:gap-2">
               {menuItems.map((item, index) => {
                 // 4. COMPARACIÓN DE ESTADO ACTIVO
                 const isActive = activeSection === item.id;
                 ...
                 return (
                   <Link 
                     ...
                     className={`
                       ...
                       ${isActive 
                         ? "bg-[#0097B2] text-black font-bold shadow-[0_0_18px_rgba(0,151,178,0.5)]" // Estilo Activo
                         : "text-gray-100 hover:text-white hover:bg-white/8" // Estilo Inactivo
                       }
                     `}
                   >
             ...
             <div className="mt-5 md:mt-6 mb-2 px-2.5 md:px-3">
               <AnimatePresence>
                 {isHovered ? (
                   <motion.span initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="text-[9px] md:text-[10px] lg:text-xs font-bold text-gray-100 uppercase tracking-widest">Extras</motion.span>
                 ) : (
