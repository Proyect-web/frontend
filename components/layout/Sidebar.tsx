...
export default function Sidebar({ logoUrl, siteTitle }: SidebarProps) {
  ...
  // URL del Dashboard
  const DASHBOARD_BASE_URL = process.env.NEXT_PUBLIC_DASHBOARD_URL || "https://goh2.vercel.app/dashboard";
  ...
  // Función para construir la URL del dashboard con el token si está autenticado
  const getDashboardUrl = () => {
    if (isAuthenticated && token) {
      // Si está autenticado, pasar el token al dashboard
      return `${DASHBOARD_BASE_URL}?token=${encodeURIComponent(token)}`;
    }
    ...
    <>
      ...
      >
        ...
        <div className="flex items-center gap-3 sm:gap-4">
          ...
          {isAuthenticated && (
            <Link href={dashboardUrl} target="_blank" className="text-white hover:text-blue-400 transition-colors">
              <User size={22} />
            </Link>
          )}
              ...
              <div className="flex flex-col gap-3 p-4 sm:p-6">
                {menuItems.map((item, index) => (
                  <Link
                    ...
                    href={item.href}
                    onClick={() => setIsMobileMenuOpen(false)}
                    className="flex items-center gap-4 p-3.5 sm:p-4 rounded-xl font-bold transition-all bg-white/5 hover:bg-white/10 text-white"
                  >
                ...
                {isAuthenticated ? (
                  <Link href={dashboardUrl} target="_blank" onClick={() => setIsMobileMenuOpen(false)} className="flex items-center justify-center gap-2 mt-4 p-3.5 sm:p-4 bg-[#0097B2] text-black font-bold rounded-xl hover:bg-[#0097B2]/90 transition-colors">
          ...
          {isAuthenticated ? (
            <Link href={dashboardUrl} target="_blank">
              <button className="hidden md:flex items-center gap-2 lg:gap-3 px-4 py-2 md:px-6 md:py-2.5 lg:px-5 lg:py-2 bg-black hover:bg-gray-800 text-white rounded-full text-sm md:text-xs lg:text-sm font-bold transition-all border border-white/10 shadow-lg hover:shadow-xl group">
                ...
                <User size={22} className="hidden lg:block group-hover:text-cyan-400" />
              </button>
            </Link>
          ) : (
            <Link href={dashboardUrl} target="_blank">
        ...
        <div className="fixed top-8 md:top-12 left-3 md:left-4 lg:left-6 z-50 flex flex-col gap-4 md:gap-5 lg:gap-6 items-start">
            ...
            <Link href="/" className="flex items-center gap-2 md:gap-3 lg:gap-4 pl-1 md:pl-2 mb-2 group">
              {logoUrl ? (
                <div className="relative w-4 h-4 md:w-8 md:h-8 lg:w-12 lg:h-12 rounded-full bg-black/50 border border-white/10 flex items-center justify-center overflow-hidden shadow-lg group-hover:scale-105 transition-transform">
                  <Image src={logoUrl} alt="Logo" width={36} height={36} className="object-contain md:w-[65px] md:h-[65px] lg:w-[70px] lg:h-[70px]" />
                </div>
              ) : (
              ...
              <AnimatePresence>
                {!hideLogoText && (
                  <motion.span initial={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -10 }} transition={{ duration: 0.3 }} className="text-xl md:text-2xl lg:text-3xl font-black text-white tracking-tight drop-shadow-md uppercase">
                    {displayTitle}
                  </motion.span>
                )}
              </AnimatePresence>
          ...
          >
            ...
            <div className="mt-5 md:mt-6 mb-2 px-2.5 md:px-3">
              <AnimatePresence>
                {isHovered ? <motion.span initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="text-[9px] md:text-[10px] lg:text-xs font-bold text-gray-100 uppercase tracking-widest">Extras</motion.span> : <div className="h-0.5 w-full bg-white/10 mx-auto rounded-full" />}
              </AnimatePresence>
            </div>
